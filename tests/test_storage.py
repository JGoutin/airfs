...
def _urandom(size):
    """Return random generated bytes.

    But avoid generating Null chars.
...
class StorageTester:
    ...
    def __init__(
        ...
        Args:
            system (airfs._core.io_system.SystemBase instance): System to test.
            raw_io (airfs._core.io_raw.ObjectRawIOBase subclass): Raw IO class.
            buffered_io (airfs._core.io_buffered.ObjectBufferedIOBase subclass):
                Buffered IOclass.
            storage_mock (tests.storage_mock.ObjectStorageMock instance): Storage mock
                in use, if any.
        ...
        self._system_parameters["storage_parameters"]["airfs.system_cached"] = system
        self._system = system
        self._raw_io = raw_io
    ...
    def _test_raw_io(self):
        ...
        if self._is_supported("write"):
            with self._raw_io(file_path, "wb", **self._system_parameters) as file:
                assert (
                    self._system.getsize(file_path) == 0
                ...
                assert file.write(content[:10]) == 10, "Raw write, written size match"
                if is_seekable:
                    assert file.write(b"\0" * 10) == 10, "Raw write, written size match"
                else:
                    assert (
                        file.write(content[10:20]) == 10
                    ), "Raw write, written size match"
                assert file.write(content[20:]) == 80, "Raw write, written size match"

                # Test: tell
                if is_seekable:
                    assert file.tell() == size, "Raw write, tell match writen size"

                    # Test write seek back and write
                    assert file.seek(10) == 10, "Raw write, seek position match"
                    assert (
                        file.write(content[10:20]) == 10
                    ), "Raw write, written size match"
                    assert file.tell() == 20, "Raw write, tell match ending position"
        ...
        with self._raw_io(file_path, **self._system_parameters) as file:
            ...
            assert (
                file.readall() == content[10:]
            ), "Raw seek 10 & read all, content match"
            assert file.tell() == size, "Raw seek 10 & read all, tell match"

            # Test: seek from current position & read_all
            assert (
                file.seek(-50, SEEK_CUR) == 50
            ), "Raw seek from current & read all, seek match"
            assert (
                file.readall() == content[-50:]
            ), "Raw seek from current & read all, content match"
            assert file.tell() == size, "Raw seek from current & read all, tell match"
            ...
            buffer = bytearray(40)
            assert file.readinto(buffer) == 40, "Raw read into, returned size match"
            assert bytes(buffer) == content[:40], "Raw read into, content match"
            assert file.tell() == 40, "Raw read into, tell match"

            buffer = bytearray(40)
            assert (
                file.readinto(buffer) == 40
            ), "Raw read into from 40, returned size match"
            assert (
                bytes(buffer) == content[40:80]
            ), "Raw read into from 40, content match"
            assert file.tell() == 80, "Raw read into from 40, tell match"

            buffer = bytearray(40)
            assert (
                file.readinto(buffer) == 20
            ), "Raw read into partially over EOF, returned size match"
            assert (
                bytes(buffer) == content[80:] + b"\0" * 20
            ), "Raw read into partially over EOF, content match"
            assert file.tell() == size, "Raw read into partially over EOF, tell match"

            buffer = bytearray(40)
            assert (
                file.readinto(buffer) == 0
            ), "Raw read into over EOF, returned size match"
            assert bytes(buffer) == b"\0" * 40, "Raw read into over EOF, content match"
            assert file.tell() == size, "Raw read into over EOF, tell match"

            file.seek(-10, SEEK_END)
            buffer = bytearray(20)
            assert (
                file.readinto(buffer) == 10
            ), "Raw seek from end & read into, returned size match"
            assert (
                bytes(buffer) == content[90:] + b"\0" * 10
            ), "Raw seek from end & read into, content match"
            assert file.tell() == size, "Raw seek from end & read into, tell match"

        # Test: Append mode
        if self._is_supported("write"):
            # Test: Appending on existing file
            with self._raw_io(file_path, mode="ab", **self._system_parameters) as file:
                file.write(content)

            with self._raw_io(file_path, **self._system_parameters) as file:
                assert (
                    file.readall() == content + content
        ...
        if is_seekable:
            ...
            with self._raw_io(file_path, "rb", **self._system_parameters) as file:
                assert (
                    file.readall() == b"\0" * 256 + b"\x01"
        ...
        if self._is_supported("write") and max_flush_size:
            ...
            with self._raw_io(file_path, "rb", **self._system_parameters) as file:
                assert file.readall() == content, "Raw Write big file, content match"
    ...
    def _test_buffered_io(self):
        ...
        with self._buffered_io(
            file_path, "rb", buffer_size=buffer_size, **self._system_parameters
        ) as file:
            assert content == file.read(), "Buffered read, not multiple of buffer size"
        ...
        with self._buffered_io(
            ...
            # Test full data read
            assert content == file.read(), "Buffered read, multiple of buffer size"
            ...
            # Test: peek:
            assert content[10:110] == file.peek(
                100
        ...
        if is_rfs_subclass:
            with self._buffered_io(file_path, **self._system_parameters) as file:
                assert (
                    file._buffer_size == file.DEFAULT_BUFFER_SIZE
            ...
            if minimum_buffer_size > 1:
                with self._buffered_io(
                    file_path,
                    buffer_size=minimum_buffer_size // 2,
                    **self._system_parameters,
                ) as file:
                    assert (
                        file._buffer_size == minimum_buffer_size
            ...
            if maximum_buffer_size:
                with self._buffered_io(
                    file_path,
                    buffer_size=maximum_buffer_size * 2,
                    **self._system_parameters,
                ) as file:
                    assert (
                        file._buffer_size == maximum_buffer_size
                    ), "Buffered, Maximum buffer size"

            # Test: Maximum buffer count
            assert calculated_max_buffers, "Buffered, calculated buffer count not 0"

            max_buffers = calculated_max_buffers * 2
            with self._buffered_io(
                file_path, mode="rb", max_buffers=max_buffers, **self._system_parameters
            ) as file:
                assert (
                    file._max_buffers == max_buffers
    ...
    def _test_system_locator(self):
        ...
        if self._is_supported("listdir") and self._is_supported("list_locator"):
            ...
            assert hasattr(
                system.head(path=self.locator), "__getitem__"
    ...
    def _test_system_objects(self):
        ...
        if self._is_supported("mkdir"):
            ...
            if self._is_supported("listdir"):
                assert (
                    dir_path0 in self._list_objects_names()
                ), 'Create directory, exists (with "/")'

            # Test: Check directory header
            assert hasattr(
                system.head(path=dir_path0), "__getitem__"
            ...
            if self._is_supported("listdir"):
                assert (
                    dir_path1 in self._list_objects_names()
                ), 'Create directory, exists (without "/")'

                # Test: Listing empty directory
                assert (
                    len(tuple(system.list_objects(dir_path0))) == 0
        ...
        if self._is_supported("write"):
            with self._raw_io(file_path, mode="w", **self._system_parameters) as file:
                # Write content
                file.write(content)
        ...
        assert hasattr(
            system.head(path=file_path), "__getitem__"
        ...
        for i in range(11):
            ...
            self._to_clean(path)
            if self._is_supported("write"):
                with self._raw_io(path, mode="w", **self._system_parameters) as file:
                    file.flush()
            elif self._storage_mock:
                # Create pre-existing file
        ...
        if self._is_supported("remove"):
            if self._is_supported("listdir"):
                assert (
                    file_path in self._list_objects_names()
                ), "Remove file, file exists"
            system.remove(file_path)
            if self._is_supported("listdir"):
                assert (
                    file_path not in self._list_objects_names()
...
def test_user_storage(storage_test_kwargs):
    ...
    Test cases are automatically generated base on user configuration, see
    "tests.conftest.pytest_generate_tests".
    ...
    # Run tests
    with StorageTester(
        unsupported_operations=unsupported_operations, **storage_test_kwargs
    ) as tester:
        tester.test_common()
...
def test_mock_storage():
    ...
    from tests_storage_package.mock import MockSystem, MockBufferedIO, MockRawIO

    # Init mocked system
    system = MockSystem()

    # Tests
    with StorageTester(system, MockRawIO, MockBufferedIO, system.client) as tester:
        # Common tests
        tester.test_common()
