...
def test_mocked_storage():
    ...
    import airfs.storage.azure_blob as azure_blob
    from airfs.storage.azure_blob import (
        _AzureBlobSystem,
        AzureBlobRawIO,
        AzureBlockBlobRawIO,
        AzurePageBlobRawIO,
    ...
    class BlobService:
        ...
        def get_blob_properties(self, container_name=None, blob_name=None):
            ...
            props.blob_type = storage_mock.head_object(container_name, blob_name)[
                "blob_type"
            ]
    ...
    class PageBlobService(BlobService):
        ...
        def create_blob(
            ...
            storage_mock.put_object(
                ...
                blob_name,
                content=content,
                headers=dict(blob_type=self.BLOB_TYPE),
                new_file=True,
            )
    ...
    try:
        ...
        tester_kwargs = dict(
            ...
            buffered_io=AzureBlobBufferedIO,
        ...
        with StorageTester(system, **tester_kwargs) as tester:
            ...
            with AzureBlobRawIO(
                tester.base_dir_path + "file0.dat", **tester._system_parameters
            ) as file:
                assert isinstance(file, AzureBlockBlobRawIO), "Azure Raw is Block raw"
        ...
        with StorageTester(system, **tester_kwargs) as tester:
            ...
            with AzureBlobRawIO(file_path, **tester._system_parameters) as file:
                assert isinstance(file, AzurePageBlobRawIO), "Azure Raw is page raw"

            # Test pre-allocating pages with page aligned content length
            with AzureBlobRawIO(
                file_path, "wb", content_length=1024, **tester._system_parameters
            ):
                pass

            with AzureBlobRawIO(
                file_path, ignore_padding=False, **tester._system_parameters
            ) as file:
                assert file.readall() == b"\0" * 1024, "Azure raw initialized"

            # Test pre-allocating pages with page unaligned content length
            with AzureBlobRawIO(
                file_path, "wb", content_length=1234, **tester._system_parameters
            ):
                pass
            ...
            with AzureBlobRawIO(
                ...
                assert (
                    file.readall() == b"\0" * 1536
                ), "Azure raw initialized and page aligned"

            # Test increase already existing blob size
            with AzureBlobRawIO(
                file_path, "ab", content_length=2345, **tester._system_parameters
            ):
                pass
            ...
            # Test not truncate already existing blob with specified content length
            with AzureBlobRawIO(
                file_path, "ab", content_length=1024, **tester._system_parameters
            ):
                pass
            ...
            with AzureBlobRawIO(
                ...
                assert (
                    file.readall() == b"\0" * 2560
                ), "Azure Raw resize do not truncate"

            # Test ignore padding parameter
            with AzureBlobRawIO(file_path, "wb", **tester._system_parameters) as file:
                file.write(b"0")

            with AzureBlobRawIO(
                file_path, "ab", ignore_padding=False, **tester._system_parameters
            ) as file:
                assert file.tell() == 512, "Azure raw tell return end of page"

            with AzureBlobRawIO(
                file_path, "ab", ignore_padding=True, **tester._system_parameters
            ) as file:
                assert file.tell() == 1, "Azure raw tell return end of data"

            # Test Buffered IO: Page unaligned buffer size rounding
            with AzurePageBlobBufferedIO(
                file_path, "wb", buffer_size=1234, **tester._system_parameters
            ) as file:
                assert (
                    file._buffer_size == 1536
        ...
        with StorageTester(system, **tester_kwargs) as tester:
            ...
            with AzureBlobRawIO(
                ...
                assert isinstance(file, AzureAppendBlobRawIO), "Azure raw is Append raw"
